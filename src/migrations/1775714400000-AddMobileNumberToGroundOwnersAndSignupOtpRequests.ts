...
export class AddMobileNumberToGroundOwnersAndSignupOtpRequests1775714400000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE "admins" ADD CONSTRAINT "CHK_admins_email_or_mobile_number" CHECK ("email" IS NOT NULL OR "mobile_number" IS NOT NULL)',
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE "admins" DROP CONSTRAINT IF EXISTS "CHK_admins_email_or_mobile_number"',
    );
    await queryRunner.query(
